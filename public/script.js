...
// Throttle function to limit the rate of emitting draw events
...
socket.on("add-score-in-match_over", (roomid, all_room_members) => {
  ...
  for (let i = 0; i < all_room_members.length; i++) {
    let item = document.createElement("div");
    let panel_item_score = document.getElementById(
      `user_${roomid}_${all_room_members[i][0]}_score`
    );
...
socket.on("reset-scores", (all_room_members, id) => {
  for (let i = 0; i < all_room_members.length; i++) {
    let panel_item_score = document.getElementById(
      `user_${id}_${all_room_members[i][0]}_score`
    );
